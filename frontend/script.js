...
function carregarCheckout() {
    ...
    // Verificar parâmetro da URL para pré-seleção de método de pagamento
    preSelecionarMetodoPagamento();
}
...
// Pré-selecionar método de pagamento baseado no parâmetro da URL
function preSelecionarMetodoPagamento() {
    const urlParams = new URLSearchParams(window.location.search);
    const metodoParam = urlParams.get('metodo');

    if (metodoParam) {
        const radio = document.querySelector(`input[name="metodo-pagamento"][value="${metodoParam}"]`);
        if (radio) {
            radio.checked = true;
            // Disparar o evento change para mostrar o formulário correspondente
...
// Função para mostrar seções do perfil
function mostrarSecao(secao) {
    // Esconder todas as seções
    const secoes = document.querySelectorAll('.profile-section');
    ...
    const secaoAtiva = document.getElementById(secao + '-section');
    if (secaoAtiva) {
        secaoAtiva.classList.add('active');
    }

    // Adicionar classe active ao nav item clicado
    const navItemAtivo = document.querySelector(`[onclick="mostrarSecao('${secao}')"]`);
    if (navItemAtivo) {
        navItemAtivo.classList.add('active');
    }
    ...
    if (secao === 'dados') {
        ...
        carregarPedidosPerfil();
    } else if (secao === 'enderecos') {
...
async function carregarDadosPerfil() {
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById("info-nome").textContent = userData.nome || 'Não informado';
            ...
            document.getElementById("info-telefone").textContent = userData.telefone || 'Não informado';
            document.getElementById("info-cpf").textContent = userData.cpf || 'Não informado';
            document.getElementById("info-data").textContent = new Date(userData.dataCadastro).toLocaleDateString('pt-BR');
        } else {
...
async function carregarEstatisticasPerfil() {
    try {
        ...
        const responsePedidos = await fetch(`${API_BASE_URL}/orders`, {
            method: 'GET',
            headers: getAuthHeaders()
        });
        ...
        // Carregar endereços para estatísticas
        const responseEnderecos = await fetch(`${API_BASE_URL}/addresses`, {
            method: 'GET',
            headers: getAuthHeaders()
        });
        ...
        if (responseEnderecos.ok) {
            document.getElementById("total-enderecos").textContent = enderecos.length;
        }
        ...
        document.getElementById("total-enderecos").textContent = '0';
    }
}

async function carregarPedidosPerfil() {
    ...
    try {
        ...
        if (response.ok) {
            if (pedidos.length === 0) {
                ...
                pedidos.forEach(pedido => {
                    ...
                    const statusMap = {
                        'PENDENTE_PAGAMENTO': { text: 'Pendente', class: 'status-pending' },
                        'PROCESSANDO_PAGAMENTO': { text: 'Processando', class: 'status-processing' },
                        'PAGO': { text: 'Pago', class: 'status-processing' },
                        'SEPARACAO_ESTOQUE': { text: 'Em Separação', class: 'status-processing' },
                    ...
                    const status = statusMap[pedido.status] || { text: pedido.status, class: 'status-pending' };

                    orderCard.innerHTML = `
                        <div class="order-header">
                            <div class="order-id">Pedido #${pedido.codPedido}</div>
                        ...
                        <div class="order-items">
                            ${pedido.itens ? pedido.itens.map(item => `
                                <div class="order-item">
                                    ...
                                    <div class="item-price">R$ ${(item.precoUnitario * item.quantidade).toFixed(2)}</div>
                                </div>
                            `).join('') : ''}
                        </div>
                        <div class="order-footer">
                            ...
                            <div class="order-actions">
                                <button class="btn-outline" onclick="verDetalhesPedido(${pedido.codPedido})">Ver Detalhes</button>
                            </div>
                        </div>
                    `;

                    listaPedidos.appendChild(orderCard);
                });
            }
        } else {
        ...
        listaPedidos.innerHTML = "<p>Erro de conexão com o servidor.</p>";
    }
}

async function carregarEnderecosPerfil() {
    const listaEnderecos = document.getElementById("lista-enderecos-perfil");
    ...
    try {
                ...
                enderecos.forEach(endereco => {
                    ...
                    addressCard.innerHTML = `
                        <div class="address-header">
                            <div class="address-title">${endereco.apelido || 'Endereço'}</div>
                            ${endereco.is_principal ? '<div class="address-badge">Principal</div>' : ''}
                        </div>
                        ...
                        <div class="address-actions">
                            <button class="btn-outline" onclick="editarEndereco(${endereco.codEndereco})">Editar</button>
                            <button class="btn-outline" onclick="excluirEndereco(${endereco.codEndereco})">Excluir</button>
                        </div>
                    `;

                    listaEnderecos.appendChild(addressCard);
                });
            }
        } else {
        ...
        console.error('Erro ao carregar endereços:', error);
        listaEnderecos.innerHTML = "<p>Erro de conexão com o servidor.</p>";
    }
}

function verDetalhesPedido(pedidoId) {
    // Implementar visualização de detalhes do pedido
    mostrarMensagem("Funcionalidade em desenvolvimento", 'error');
}
